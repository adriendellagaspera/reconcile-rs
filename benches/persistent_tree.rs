...
//! reproduction commands. This target measures the COW-specific costs that commit cannot express:
//! snapshot acquisition, zero-copy iteration/range reads, writes while the current version is
//! retained, and retained-version heap growth.
...
/// version. The latter is the condition that makes the next mutation exercise `Arc::make_mut`'s
/// path copy; older versions add the memory pressure a long-lived snapshot workload creates.
fn retained_history(
    ...
    for &n in SIZES {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(n), &n, |b, _| {
            b.iter(|| {
                let sum = tree
                    .iter()
                    .fold(0u64, |acc, (k, v)| acc.wrapping_add(*k as u64).wrapping_add(*v as u64));
                black_box(sum)
            });
        });
    }
...
/// Prints requested live-heap growth from retaining successive COW versions. This deliberately
/// mirrors `system::heap_footprint`: it is a floor on RSS, excludes allocator rounding and
/// fragmentation, and exists to compare retained-version shapes rather than claim exact RSS.
...
criterion_group!(
    ...
    iteration,
    range_iteration,
    mutation_with_retained_snapshots,
    retained_snapshot_memory,
);
