...
# The counted, machine-independent half needs the test-only seam it reads.
cargo bench --bench contention --features internal-testing -- 'no_such_benchmark'
...
per the recipe above. The timed half was measured **without** `internal-testing`, so no counter runs
