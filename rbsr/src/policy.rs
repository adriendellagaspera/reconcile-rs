...
/// wins on bytes outright — a set-shaped store (`V = ()`) or a payload of a few bytes, and at five
/// of the eight cases rather than all of them. Above it, it still wins the wall clock wherever the
/// RTT clears the second — every swept case at 8-byte values, and any payload while `d` stays
/// small. **Keep the default when values are large *and* differences are many**: there the extra
/// elements cost more transmission time than the round trips they buy, and `d`, not `n`, is the
/// axis that decides it. Both figures assume a lossless link at line rate; under loss the binding
/// term is `reconcile_interval` per lost datagram instead (`SOTA.md` §2.2).
///
