...
impl Comparison {
    ...
    /// The local fingerprint's low limb — **crate-private on purpose**: this is precisely the
    /// oracle the law above forbids a policy from reading, exposed only so
    /// [`oracle_probe`](crate::oracle_probe)'s counter-examples can be built inside this crate
    /// and nowhere else.
    #[cfg(feature = "internal-testing")]
    pub(crate) const fn local_fingerprint_limb(&self) -> u64 {
        self.local.fingerprint().0[0]
    }
}
...
pub(crate) fn shared_cutoffs(comparison: Comparison) -> Option<Decision> {
